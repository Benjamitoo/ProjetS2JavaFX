...
import controleur.AjoutOperateurControleur;
...
public class AjoutOperateur {

    private Stage stage;
    private TextField idField, nField, pField;
    private Button evenementButton, creerButton, retourButton;

    public AjoutOperateur(Stage primaryStage) {
        this.stage = primaryStage;
        buildUI();
    }

    private void buildUI() {
        stage.setTitle("Ajout d'un Opérateur");
    ...
    try {
        new AjoutOperateurControleur().creerOperateur(
            idField.getText(),
            nField.getText(),
            pField.getText()
        );
        ...
        alert.setContentText("Opérateur créé avec succès !");
        alert.showAndWait();
        ...
        alert.setContentText("L'opérateur n’a pas pu être créé.\nDétail : " + ex.getMessage());
        alert.showAndWait();
    }
});
